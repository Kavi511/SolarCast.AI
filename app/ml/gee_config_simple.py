"""
Simplified Google Earth Engine Configuration
===========================================

A lightweight version that only requires earthengine-api package.
This avoids the Windows Long Path issues with geemap.
"""
...
class SimpleGEEConfig:
    """Simplified Google Earth Engine configuration without geemap dependency"""
    ...
    def fetch_satellite_image(self,
        ...
        try:
            ...
            # Validate date is not in the future (Sentinel-2 data is only available for past dates)
            from datetime import datetime, timedelta
            date_obj = datetime.strptime(date, "%Y-%m-%d")
            today = datetime.now().date()
            
            # If date is in the future, use yesterday's date instead
            # Sentinel-2 images typically have a 1-3 day processing delay
            if date_obj.date() > today:
                # Use 3 days ago to ensure data availability (accounting for processing delay)
                date_obj = datetime.combine(today - timedelta(days=3), datetime.min.time())
                date = date_obj.strftime("%Y-%m-%d")
                logger.warning(f"Future date requested. Using {date} instead (3 days ago to ensure data availability).")
            elif date_obj.date() == today:
                # If today, use yesterday to ensure data is available
                date_obj = datetime.combine(today - timedelta(days=1), datetime.min.time())
                date = date_obj.strftime("%Y-%m-%d")
                logger.info(f"Today's date requested. Using {date} instead (yesterday to ensure data availability).")
            
            # Add 1 day buffer to end_date to avoid empty date range error
            end_date = (date_obj + timedelta(days=1)).strftime("%Y-%m-%d")
            
            if collection_type == 'sentinel2':
                collection = self.get_sentinel2_collection(date, end_date)
                rgb_bands = ['B4', 'B3', 'B2']  # Red, Green, Blue
            elif collection_type == 'landsat8':
                collection = self.get_landsat8_collection(date, end_date)
            ...
            try:
                ...
                if collection_size == 0:
                    logger.warning(f"No {collection_type} images found for {date}")
                    return self._generate_mock_satellite_data()
            except Exception as e:
                logger.warning(f"Error checking collection size: {e}")
                return self._generate_mock_satellite_data()
...
gee_config = SimpleGEEConfig()

def get_gee_config() -> SimpleGEEConfig:
    """Get the global GEE configuration instance"""
    return gee_config

def initialize_gee(project_id: str = 'instant-text-459407-v4') -> SimpleGEEConfig:
    """Initialize GEE with custom project ID"""
    global gee_config
    gee_config = SimpleGEEConfig(project_id)
    return gee_config
